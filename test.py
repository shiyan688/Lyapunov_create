...
import numpy as np
import random
def symbolic_orthogonalization(grad_V, vectors):
    """对给定的梯度和向量进行符号化正交化"""
    # 将输入向量转换为符号形式
    grad_V_sym = sp.Matrix(grad_V)
    vectors_sym = [sp.Matrix(v) for v in vectors]
    
    def project(u, v):
        """将向量 v 投影到向量 u 上"""
        u_norm_squared = u.dot(u)
        projection_coefficient = u.dot(v) / u_norm_squared
        projection_vector = projection_coefficient * u
        return projection_vector

    orthogonal_vectors_sym = []
    for v in vectors_sym:
        # 使当前向量与梯度正交
        v = v - project(grad_V_sym, v)
        orthogonal_vectors_sym.append(v)

    return orthogonal_vectors_sym

def check_orthogonality_and_validate(grad_V, orthogonal_vectors):
    """检查内积并带入随机值进行验证"""
    for v in orthogonal_vectors:
        inner_product = sp.simplify(grad_V.dot(v))
        
        print(f"内积: {inner_product}")
        
        if inner_product != 0:
            # 如果内积不为0，带入随机值验证
            random_values = {str(var): np.random.rand() for var in v.free_symbols}
            evaluated_inner_product = inner_product.evalf(subs=random_values)
            print(f"带入随机值后的内积: {evaluated_inner_product}")
...
orth_vectors = gram_schmidt_orthogonalization(grad_V,  random_vectors)
check_orthogonality_and_validate(grad_V, orth_vectors)